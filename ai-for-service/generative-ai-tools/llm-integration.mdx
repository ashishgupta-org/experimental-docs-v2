...
> **Note:** Set up all required variables before configuring the LLM. See [App Variables](/ai-for-service/generative-ai-tools/app-settings/variables/using-bot-variables.md).
...
> **Note:** The Platform supports Azure OpenAI and OpenAI integrations for the Chat Completions API only. To use the Responses API, configure a Custom LLM.

> **Note:** The Platform regularly integrates new models from providers. To use a model not yet available as a pre-built integration, see [Add a New Model to a Pre-built Integration](#add-a-new-model-to-a-pre-built-integration).
...
Next: enable [GenAI Features](genai-features.md).
...
Next: add [Prompts](prompts-library.md).
...
> **Note:** For private/on-premises deployments, the trust policy must point to your internal AWS IAM role.
...
> **Note:** Without this step, the Platform cannot assume your IAM role. Both your AWS account and Kore.ai's environment must explicitly trust each other for cross-account access.
...
> **Note:** This feature is available only for Automation AI and Search AI.
...
> **Note:** The Platform offers generative AI features for English and non-English NLU and AI Agent languages.
...
3. On the **Auth** tab, select an existing authorization profile or create a new one. See [App Authorization Overview](/ai-for-service/generative-ai-tools/app-settings/dev-tools/bot-authorization/bot-authentication.md).

   > **Note:** OAuthv2.0 and Kerberos SPNEGO auth profiles are supported for Custom LLM integration.
...
Next: use these models in [GenAI Features](genai-features.md).
...
> **Note:** This change affects only the in-development copy of the app. Changes apply to the published version when you next publish the app with NLP configurations.
