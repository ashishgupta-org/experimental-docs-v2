...
- **Voice:** Supports the [Voice Gateway](../channels/voice-gateway/configure-voice-gateway.md) channel for GenAI features including Agent Node, with Deepgram as the supported TTS engine.
- **Chat:** Delivers real-time LLM response streaming for [Web/Mobile SDK](../sdk/bot-sdk-introduction.md) channels. Agent Node and Prompt Node stream responses token-by-token.
...
![Default streaming prompt selection](/ai-for-service/images/default-streaming.png)

### Create a Custom Streaming Prompt

See [How to Add a Custom Prompt](prompts-library.mdx) and enable the **Stream Response** toggle.
...
> **Note:** These benchmarks were conducted under specific scenarios. Performance varies by environment. Conduct your own testing before enabling streaming in production.
...
![LLM Usage Logs with TTFT column](/ai-for-service/images/streaming-logs.png)

![Streaming response detail view](/ai-for-service/images/streaming-details.png)
...
> **Note:** Streaming quality depends heavily on prompt design. LLMs are subject to hallucination—ensure prompts are accurate and aligned with desired output before using in production.
