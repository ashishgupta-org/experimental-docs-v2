...
* **API Endpoint**: Provide the complete API endpoint URL from which Agent AI fetches the data. For example, Salesforce Apex REST endpoint for retrieving case information.
    ...
    > **Note:** You can add static query parameters that are not associated with the Salesforce Case object directly to the API endpoint. For example, https://your-domain.my.salesforce.com/services/apexrest/case?isSorted=true
...
* **API Query Params (JSON)**: Specify key-value pairs to be sent as query parameters in JSON format.
 Example:
    ...
    > **Note:** Configure the **Case Object** field names as values in **API Query Params** and **API Body** Objects. During runtime the API call is made with the actual values. For example, if **Id** is configured, the dynamic value of the Salesforce Case ID is used at runtime.
...
7. Select **Activation** and activate the page as per your requirement.
    ...
    > **Note:** The **Email** section appears on the **Case** page only after **Email-to-Case** is successfully configured in your org.
