...
> **Note:** This integration process also works for Genesys Desktop App.
...
| Capture AI Agent Information in Kore Data Table | Secure Custom Data / KvpConfig | Use this to pass sensitive information to Kore AI Agent. | [https://docs.kore.ai/xo/sdk/sdk-security/json-web-encryption-jwe](../../../sdk/sdk-security.md#json-web-encryption-jwe) |
| Create an Interaction Widget in Genesys for Agent AI | Interaction Widget | The Interaction Widget displays the Agent AI iframe within the Genesys Agent Desktop. | [https://help.mypurecloud.com/articles/set-up-an-interaction-widget-integration](https://help.mypurecloud.com/articles/set-up-an-interaction-widget-integration) |
...
![architecture-diagram-chat](../kore-data-table/architecture-diagram-chat-1.png)

**Voice**

![architecture-diagram-voice](../kore-data-table/architecture-diagram-voice-2.png)

## Interaction Sequence

![interaction-sequence](../kore-data-table/interaction-sequence-3.png)
...
> **Note:** The configuration data is stored directly in the Custom Fields of the Data Table, not in a Data Table row. A single Data Table represents a configuration of AgentAssist. The Default Values of the Custom Fields contain the necessary configuration data to load the AgentAssist widget.
   ...
   ![create-data-table](/ai-for-service/agentai/integration/genesys/images/create-data-table.png)
    ...
    5. Click the 'pencil' icon to edit a data table.
       ![pencil-icon](/ai-for-service/agentai/integration/genesys/images/pencil-icon.png)
        ...
        > **Note:** Do not click on the data table name.
...
[Third-Party Configuration](./../../configuration/third-party-configuration.md) maps Agent AI bots to specific queues. Within this configuration, you can define the bot's language, custom data configuration, or secure custom data configuration.
...
Refer to [this doc](./../../configuration/third-party-configuration.md) for instructions on creating records in Third-Party Configuration for an Agent AI Bot.
...
> **Note:** The **Values** should be **base64 encoded**.

![third-party-configuration](/ai-for-service/agentai/integration/genesys/images/third-party-configuration.png)

> **Note:** Ensure the **Third Party Configuration** setting is enabled in **Widget Settings** > **Agent AI General Settings**.
...
> **Note:** Multiple records with different AI Agent credentials shouldn't be entered into the Kore Data Table for a single QueueIdentifier.
...
3. Click **New Table**.
![new-table](../kore-data-table/new-table-4.png)
...
4. Enter the following details in the **New Table** page:
    ...
    ![multiple-columns](../kore-data-table/multiple-columns-5.png)
    ...
    4. **Indexes** for the table.
        ...
        * **Column & Sort Order** – List of columns to be included in the index; you can select multiple columns and specify the sort order (ascending or descending) for each of the selected columns. (Select QueueIdentifier with ascending order.)
        ![queue-finder](../kore-data-table/queue-finder-6.png)
    ...
    6. **App Assignments** to let apps access data in this table.
        ...
        ![create-new-app](../kore-data-table/create-new-app-7.png)
        ...
        2. Select the **Read**, **Write**, and **Delete** permissions, as required.
            ...
            ![apps-new-app](../kore-data-table/apps-new-app-8.png)

    7. **Process Assignments** (not applicable).
5. Click **Create** to create the new data table.
![create-new-data-table](../kore-data-table/create-new-data-table-9.png)
...
![app-write-access](../kore-data-table/app-write-access-10.png)
...
6. Insert the following data into the Data Table by following this [doc](./../../../apis/automation/data-insert.md). Use the JWT Token created in the previous step.
...
![single-queue-example](../kore-data-table/single-queue-example-11.png)

**Multiple Queues Example**
![multiple-queues-example](../kore-data-table/multiple-queues-example-12.png)
...
2. Go to **Admin** > **Integrations** > **OAuth**.
   ![single-queue-example](../kore-data-table/single-queue-example-13.png)
3. Click **Add Client**.
...
5. Select **Token Implicit Grant (br /owser)** in the **Grant Types** section.
   ![token-implicit-grant](../kore-data-table/token-implicit-grant-14.png)
...
![authorized-redirect-uris](../kore-data-table/authorized-redirect-uris-15.png)
...
3. If you have not created an Interaction Widget for Agent AI, install a new **Interaction Widget** by clicking the **Integrations** tab on the right side of the page.
   ![genesys-cloud-integrations](../kore-data-table/genesys-cloud-integrations-16.png)

   ![interaction-widget](../kore-data-table/interaction-widget-17.png)
...
![interaction-widget-configuration](../kore-data-table/interaction-widget-configuration-18.png)
...
2. Build **x_metadata**:
    ...
    2. Click **Data** > **Data Tables,** and copy the **Data Table** name:
       ![copy-data-table-name](../kore-data-table/copy-data-table-name-19.png)
    3. Click **Apps**, and click one of the app names that has "read" access:
       ![click-apps-read-access](../kore-data-table/click-apps-read-access-20.png)
    4. Copy the **Client ID** and **Client Secret** values.
       ![client-id-client-secret-values](../kore-data-table/client-id-client-secret-values-21.png)
    ...
    6. Generate a **JSON object** with the specified structure, and then convert it into a URL-encoded string:
        ...
        ```
            Reference:
            ![java-script-demo](../kore-data-table/java-script-demo-22.png)
...
![group-filtering](../kore-data-table/group-filtering-23.png)
...
![assigned-permissions](../kore-data-table/assigned-permissions-24.png)
...
![create-queue](../kore-data-table/create-queue-25.png)
...
4. Enter/select the following details in the **Create Queue** page:
    ...
    * **Peer Id** (optional): Enter a unique ID that can be used to identify the queue from an external platform.
    ![peer-id](../kore-data-table/peer-id-26.png)
...
![audio-monitoring-toggle](../kore-data-table/audio-monitoring-toggle-27.png)
...
3. Click the **three dots** next to **Flows: Inbound Call**, and select **Inbound Message**.
   ![inbound-message](../kore-data-table/inbound-message-28.png)
4. Click the **+Add** button to create an **Inbound Message Flow**.
   ![create-inbound-message-flow](../kore-data-table/create-inbound-message-flow-29.png)
5. Click **Create Flow**. The final architect flow looks like the following screenshot:
![create-flow](../kore-data-table/create-flow-30.png)
...
3. Click **New Configuration**.
   ![messenger-new-configuration](../kore-data-table/messenger-new-configuration-31.png)
4. Enter a name and description.
   ![messenger-configuration-name-description](../kore-data-table/messenger-configuration-name-description-32.png)
...
5. Click the **Appearance** tab and complete the following information:
    ...
    * Under **Select your Supported Languages**, click the **Select language(s)** list and choose the languages that you want to support in the Messenger interface.
        ...
        > **Note:** The same language should be configured in the **Kore AI Agent**.

        ![select-supported-languages](../kore-data-table/select-supported-languages-33.png)

    * Under **Select Default Language**, click the **Select language** list and choose the default language.
    ![default-language](../kore-data-table/default-language-34.png)
...
6. Click the **Apps** tab and complete the following steps:
    * Under **Clear Conversation**, turn on the toggle button. This is required for the **Agent AI Conversation Summary** feature to work.
      ![clear-conversation-toggle](../kore-data-table/clear-conversation-toggle-35.png)
...
3. Click **New Deployment**.
   ![new-messenger-deployment](../kore-data-table/new-messenger-deployment-36.png)
4. Enter a name and description.
   ![messenger-deployment-details](../kore-data-table/messenger-deployment-details-37.png)
...
9. Copy the Messenger snippet and deploy the Messenger snippet to your website.
    * If you are building a custom Messenger, copy the deployment key.
        ...
        > **Note:** Once the above mentioned website page loads, the messenger window appears.

        ![messenger-window](../kore-data-table/messenger-window-38.png)
...
3. Enter "audiohook" in the search bar.
   ![audiohook](../kore-data-table/audiohook-39.png)
...
4. Install a new audiohook app by clicking the **Integrations** button on the top-right corner.
   ![audiohook-monitor](../kore-data-table/audiohook-monitor-40.png)
5. Open the **Audiohook**, and go to the **Configuration** tab.
   ![audiohook-configuration](../kore-data-table/audiohook-configuration-41.png)
...
6. Follow these steps to get the **Connection URL**:
    ...
    4. Click the **Copy** button against the autogenerated **Connection URL** field.
       ![configure-sip-trunk](../kore-data-table/configure-sip-trunk-42.png)
...
8. Add the following details in the **Advanced** tab:
    ...
    ```
    > **Note:** Create an OAuth using the **Client Credentials** grant type, and use the resulting **Client ID** and **Client Secret** for configuration.
...
![audiohook-credentials](../kore-data-table/audiohook-credentials-44.png)
...
1. Sign in to [Genesys Cloud](https://apps.mypurecloud.com/).
2. Go to **Admin** > **Architect** > **Architect**.
![genesys-architect](../kore-data-table/genesys-architect-45.png)
3. Click the **+Add** button to create an **Inbound Call Flow**.
![create-inbound-call-flow](../kore-data-table/create-inbound-call-flow-46.png)
4. Click **Create Flow**. The final architect flow looks like the following screenshot:
![final-architect-flow](../kore-data-table/final-architect-flow-47.png)
...
> **Note:** If a URL-encoded JSON is provided in the "x_passthru_metadata", use the JavaScript option in the Message node. Then, parse the value of *context.session.UserContext.customData.internalMetaData* before accessing its content.
...
1. Sign in to [Genesys Developer Applications](https://developer.genesys.cloud/devapps/web-chat-messenger).
    ...
    > **Note:** To start chat simulation, you must have access to the Genesys Developer portal (an account with Genesys).

    ![chat-simulation](../kore-data-table/chat-simulation-48.png)

2. Click **Web Messenger**, select the **Deployments**, and then click **Start Chat** to initiate the chat request.
   ![chat-simulation-web-messenger](../kore-data-table/chat-simulation-web-messenger-49.png)

3. Enter a message on the **Message Us** window, and then click the **Send** icon.
   ![chat-simulation-message-use](../kore-data-table/chat-simulation-message-use-50.png)
...
![interaction-widget-icon-chat](../kore-data-table/interaction-widget-icon-chat-51.png)
...
1. In **Call Routing**, assign the **Architect flow** created in **[Step-4](#steps-to-create-a-call-architect-flow)** to a Genesys Phone Number.
   ![call-routing](../kore-data-table/create-call-route-53.png)

   ![create-call-route](../kore-data-table/create-call-route-53.png)
...
![call-interaction-details](../kore-data-table/call-interaction-details-54.png)
...
5. Click **Save**.
![default-contextual-panel](../kore-data-table/default-contextual-panel-55.png)
