...
Use the configurations provided below as general guidelines, as there could be some differences based on the version of the Genesys you are using.

---

## Overview

Kore.ai's AI Agent serves as the central point of interaction for the users. In addition to its regular capabilities, the AI Agent acts as a proxy or intermediary between the users and the Genesys agents to hand over conversations seamlessly. The AI Agent facilitates communication and interaction between the user and the Genesys agent using the chat module.
...
As of now, the following features are not supported and will be available in upcoming releases:
...
1. Log in to your Genesys Cloud account. The login URL will be visible in your browser's address bar, for example, [https://login.mypurecloud.com](https://login.mypurecloud.com/).
2. Copy the URL and keep it for future use.
3. Alternatively, you can get your region-specific Genesys Cloud login URL by referring to [Genesys Cloud region URLs](https://help.mypurecloud.com/articles/aws-regions-for-genesys-cloud-deployment/#tab2).

---

### Step 2: Retrieve the Organization ID

1. Follow the steps in [Find your Organization ID](https://help.mypurecloud.com/faqs/how-do-i-find-my-organization-id/).
...
3. In the **Apps** tab, set **Conversation Disconnect** to _Display conversation status and disconnect session_, and click **Save**.
...
8. Click **Save** to generate the Deployment Key, then copy it and keep it for future use.

---

### Step 6: Add the WebSocket URL

> **Note:** If you are configuring WebChat, skip to Step 7: Enable the Genesys Agent.

1. Go to [Genesys WebSocket Region Info](https://developer.genesys.cloud/commdigital/digital/webmessaging/websocketapi#connecting) to find your region's Genesys WebSocket URL.
2. Locate the WebSocket address that matches your Genesys Cloud region.
3. Copy the WebSocket URL (for example, `wss://streaming.mypurecloud.com/v2/websocket`) and keep it for future use.
...
4. Click the **Configurations** tab and enter the following credentials for the Genesys Agent:
   - **Genesys Cloud login URL**: URL of your Genesys AWS Region, which will allow the platform to route the conversation requests to the provided Genesys AWS region. This is a **mandatory field**.
   ...
   ![Enter Genesys agent credentials](../images/genesys-at.png)
...
7. Click **Manage** to map the channels you want the Genesys agent to hand over the conversations. To learn more about channel enablement, see [Adding channels to your app](../../../channels/adding-channels-to-your-bot.md).
...
2. Create a new Agent Transfer Node task. For example, create a _PlatformAgent_ task with the _AgentTransfer_ node and configure it to use the Genesys agent. For more information, read the [Agent Transfer Node](../../../automation/use-cases/dialogs/node-types/working-with-the-agent-transfer-node.md) article.

   ![New Agent Transfer Node task](../images/configuring-the-genesys-agent-img4.png)
...
6. Now, the connection between the Genesys agent and the user has been established, and the conversation continues.
   ...
   ![Connection established between Genesys agent and user](../images/configuring-the-genesys-agent-img5.png)
...
**Sample Script to set metadata using a Script node:**
...
![Genesys Agent](../images/genesys2.png)
...
![Genesys Agent](../images/genesys5.png)
...
- In the GetParticipantData action, create flow variables and then map the relevant metadata attributes to these flow variables.
- Reference these flow variables as needed in the Transfer to the ACD node to ensure the metadata is properly utilized during the agent transfer.

![Genesys Agent](../images/genesys4.png)

![Genesys Agent](../images/genesys3.png)

![Genesys Agent](../images/genesys6.png)
...
> **Note:** The queue name in the Entity node accepts underscores (_) only when Auto Correct is turned off. We recommend either avoiding underscores in the queue name or turning off Auto Correct in the Entity node to ensure dynamic queuing works as expected.
...
2. In the Genesys Architect Inbound Message Flow, add a **Get Participant Data** action before the **Transfer to ACD** node to retrieve the QueueName from the metadata and assign it to a flow variable.
   ...
   > **Note:** Ensure that the attributeName matches the exact key used in the platform's metadata. For clarification, you can refer to the screenshots below.
...
![Genesys Agent](../images/genesys1.png)

![Genesys Agent](../images/genesys8.png)

![Genesys Agent](../images/genesys7.png)
