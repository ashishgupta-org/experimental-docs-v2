...
In the Experience Flow, add an **Automation** node and attach the app to trigger when ZCC transfers a call to Kore. See [Create Flows](../../../flows/create-flows.md) and [Automation node](../../../flows/node-types/automation.md).
...
8. Click **Attach Flow** and attach the flow from Step 1. See [Attach Flow](../../../channels/voice-gateway/configure-voice-gateway.md#attach-flow).
...
13. Copy the `curl` from the [Conversation Details and Summary API](../../../apis/automation/conversation-details-and-summary.md).
14. In Postman, paste the `curl` and replace the following values:
    ...
    **JWT Token** — Follow the [JWT token generation guide](../../../apis/automation/api-introduction.md#generating-the-jwt-token).
