...
![Voice Gateway Page](/ai-for-service/channels/voice-gateway/images/vg-page.png)
...
![Get New Phone Number](/ai-for-service/channels/voice-gateway/images/buy-phone-number.png)
...
1. The number appears in the SIP DID numbers list-[Create Start Flows](/ai-for-service/channels/voice-gateway/flows/create-flows.md#create-a-start-flow) and associate them with the toll-free number.
...
2. Select a **Start Flow** you want to add to this number and select **Done**. You can add a start flow by clicking **+ New Start Flow**. [Learn more](/ai-for-service/channels/voice-gateway/flows/create-flows.md#create-a-start-flow).
    ...
    1. **Product Selection**: Select the product for SIP Trunk configuration. You can select from the following options:
        ...
        - **Network**: To configure the Network, you can select one of the following:
            ...
            - Under **Domain Name**, provide the following:
                - **Fully Qualified Domain Name (FQDN)**: The domain name specifies all domain levels, including the top-level domain and the root zone. [Learn more](/ai-for-service/channels/voice-gateway/deployment-and-operations.md#ips-ports-and-protocols).
        ...
        - Enter the **SIP Headers**. You can include all available agent data in the SIP headers, enabling customers to use only the parameters relevant to their needs. The system lists the parameters as key-value pairs:
            ...
            <Note> The caller number specified in the [Script Task](/ai-for-service/channels/voice-gateway/flows/node-types/script-task.md) is passed through the SIP headers when a third-party desktop application transfers the call to an agent.</Note>
        ...
        **Select the SIP Trunk connection method**: Select the method based on your third-party vendor’s requirements ([SIPREC](#siprec) or [WebSocket](#websocket)).
        ...
        - If you select SIPREC:
            ...
            - **Record the calls**: Enable or disable call recordings for third-party Agent Desktop integrations. Access these recorded calls using a [public API](/ai-for-service/channels/voice-gateway/apis/contact-center/api-list.md#call-recording-apis).
            - (Optional) By default, the system selects **E.164 Syntax**. Selecting this option prepends a + before the DID number.
            ...
            - **Call control parameters**: Define the parameters to control the call behavior. Select **Add Parameter**, enter the **Parameter Name** and **Value**, and select **Save**. [Learn more](/ai-for-service/channels/voice-gateway/speech-customization.md#introduction-to-call-control-parameters).
            - Enter the **SIP Headers**. You can include all available agent data in the SIP headers, enabling customers to use only the parameters relevant to their needs. The system lists the parameters as key-value pairs:
                - X-AgentName: `{{agentName}}`
                - X-AgentPhoneNumber: `{{agentPhoneNumber}}`
                - X-AgentEmailID: `{{agentEmailId}}`
        ...
        - If you select **WebSocket**:
            ...
            - **Record the calls**: Enable or disable call recordings for third-party Agent Desktop integrations. Access these recorded calls can using a [public API](/ai-for-service/channels/voice-gateway/apis/contact-center/api-list.md#call-recording-apis).
            - **Call control parameters**: Define the parameters to control the call behavior. Select **Add Parameter**, enter the **Parameter Name** and **Value**, and select **Save**. [Learn more](/ai-for-service/channels/voice-gateway/speech-customization.md#introduction-to-call-control-parameters).
...
2. Select a **Start Flow** to add to individual numbers and select **Done**. You can add a start flow by selecting **+ New Start Flow**. [Learn more](/ai-for-service/channels/voice-gateway/flows/create-flows.md#create-a-start-flow).
...
Voice Gateway supports integration with third party ASR, TTS, and Voice Biometrics providers. [Learn more](/ai-for-service/channels/voice-gateway/voice-gateway/third-party-asr-tts-support.md)
...
1. Create a new Flow.
2. After the Start node, drag an [IVR Menu](/ai-for-service/channels/voice-gateway/flows/node-types/ivr-menu.md) node.
...
4. For each language option, connect the number key (for example, "1") to a new [Script node](/ai-for-service/channels/voice-gateway/flows/node-types/script-task.md). This is the key step where the language is set.
...
For more details on these advanced settings, refer to the [Call Control Parameters](/ai-for-service/channels/voice-gateway/voice-gateway/speech-customization.md#supported-call-control-parameters).
...
1. Open the [Dialog Task](/ai-for-service/channels/voice-gateway/automation/use-cases/dialogs/navigating-dialog-tasks.md) and select a [Message Node](/ai-for-service/channels/voice-gateway/automation/use-cases/dialogs/node-types/nodes-transitions.md#message-or-bot-response-node), [Entity Node](/ai-for-service/channels/voice-gateway/automation/use-cases/dialogs/node-types/nodes-transitions.md#entity-node), or [Confirmation Node](/ai-for-service/channels/voice-gateway/automation/use-cases/dialogs/node-types/nodes-transitions.md#confirmation-node).
...
Define prompt to be played when user input isn't received within the time-out period.
    ![Timeout Prompt](/ai-for-service/channels/voice-gateway/images/timeout-prompt.png)

### Barge-in

Define whether user input is allowed while a prompt is in progress. By default, this option is un-available.
    ![Barge In](/ai-for-service/channels/voice-gateway/images/barge-in.png)

### Timeout

Define the maximum wait time to receive user input. The maximum wait time is 60 seconds.
    ![Timeout](/ai-for-service/channels/voice-gateway/images/timeout.png)

### No. of Retries

Define the maximum number of retries allowed.
    ![No.of Retries](/ai-for-service/channels/voice-gateway/images/no-of-retries.png)
