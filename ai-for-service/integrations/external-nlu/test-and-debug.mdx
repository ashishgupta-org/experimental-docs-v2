...
![Debug Log - intent identified using External NLU is initiated](../images/external-nlu-test-and-debug-img1.png)
...
- Intent detected by the external NLU engine is always marked as a **Definitive Match**.
  ...
  ![Intent detected by the External NLU engine](../images/external-nlu-test-and-debug-img2.png)
...
- Hover over the information icon to view the **K-Traceid** and **X-Traceid** for each interaction. K-Traceid tracks and debugs platform activities; X-Traceid is a unique per-message identifier included in platform logs.
  ...
  ![K-Traceid and X-Traceid](../images/ktrace.png)

- Under **Session Context & Variables**, inspect intent and entity variables in detail.

  ![Session Context & Variables](../images/external-nlu-test-and-debug-img3.png)

  ![Custom Variables](../images/external-nlu-test-and-debug-img4.png)
...
- **Intent detected by external engine:**
  ...
  ![Intent detected by external engine](../images/external-nlu-test-and-debug-img5.png)
...
- Click **View Response** to see the raw API response from the external NLU engine (available whether or not intent was detected).
  ...
  ![View the API response in JSON format](../images/external-nlu-test-and-debug-img6.png)
...
- **Intent not detected by external engine** — Kore.ai NLU acts as fallback. NLP analysis shows results from all three Kore.ai engines plus ranking and resolver. See [Ranking and Resolver](../../../automation/natural-language/training/ranking-and-resolver.md).
  ...
  ![Intent not detected by external engine](../images/external-nlu-test-and-debug-img7.png)
...
- **Intent detected but not mappable** — Kore.ai NLU acts as fallback. NLP analysis displays a message with the API response containing detected intents and parameters.
  ...
  ![Intent detected but cannot be mapped](../images/external-nlu-test-and-debug-img8.png)
...
![ExtNLU Matched intent score](../images/external-nlu-test-and-debug-img9.png)
...
![Intents and Entities identified by External NLU](../images/external-nlu-test-and-debug-img10.png)
...
![Failed External NLU API calls](../images/external-nlu-test-and-debug-img11.png)
...
| API failure | If External NLU API returns an error or is unreachable, a fallback intent activates with the response: _Error in continuing the conversation due to incorrect bot definition._ See [Standard Responses](../../../automation/intelligence/conversation-management/default-conversations.md#standard-responses). |
