...
3. Open or create a dialog task (e.g., `Support Call Back`) and add an **Agent Transfer** node.
   ...
   ![Add Agent Transfer node](../images/configure%20agent-transfer-img1-at-node.png)
...
8. Enter the Callback URL:
   ...
   2. Copy the ngrok forwarding URL (format: `https://XXXXXXaa.ngrok.io`).
      ...
      ![NGROK forwarding URL](../images/configure%20agent-transfer-img2-ngrok.png)
...
9. Open the dialog task and the agent transfer node to verify the configuration is saved. Select the required Events.
   ...
   ![Agent transfer node configuration](../images/configure%20agent-transfer-img3-at-node-config.png)
...
11. Edit the BotKit SDK files:
    ...
    **`livechatapi.js`**
    ...
    ![livechatapi.js](../images/configure%20agent-transfer-img4-at-livechatapi.png)
    ...
    **`config.json`**
    ...
    ![config.json](../images/configure%20agent-transfer-img5-at-configjson.png)

    **`LiveChat.js`**
    - Set the `appId` and `appName` from your app.

    ![LiveChat.js](../images/configure%20agent-transfer-img6-at-livechat.png)

    **`app.js`**
    - Add: `sdk.registerBot(require('./LiveChat.js'))`
    - Remove any existing `sdk.registerBot` statements that are not needed.

    ![app.js](../images/configure%20agent-transfer-img7-at-appjs.png)
...
4. When a user initiates chat, the app transfers to an agent and notifies the user.
   ...
   ![Chat initiation](../images/configure%20agent-transfer-img8-at-chat1.png)

5. The agent receives a notification on LiveChat.

   ![Agent receives notification](../images/configure%20agent-transfer-img9-at-chat2.png)

6. The connection is established and the conversation continues.

   ![Connection established](../images/configure%20agent-transfer-img10-at-chat3.png)
