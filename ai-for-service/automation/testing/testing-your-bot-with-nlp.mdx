...
sidebarTitle: Utterance Testing
---
...
Extracts terms from the utterance and maps them to the KG to fetch relevant paths. Paths covering more than a threshold number of terms are shortlisted. A path with 100% term coverage and a matching FAQ is a perfect match. See [Knowledge Graph Training](../knowledge-ai/knowledge-graph-training.mdx).
...
**R&R V2** ranks only definitive matches from ML and KG (not FM) without rescoring. The R&R window shows the winning intent, ranking score, eliminated intents and their reasons, and the final result. See [Ranking and Resolver](../natural-language/training/ranking-and-resolver.mdx).
...
See [Knowledge Graph Training](../knowledge-ai/knowledge-graph-training.mdx).
