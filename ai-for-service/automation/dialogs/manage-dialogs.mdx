...
<Note>For optimal performance, limit dialog tasks to 50 or fewer per app. Exceeding this may cause sluggish UI response and increased latency.</Note>

Go to **Automation > Dialogs** and click **Create Dialog**. Choose one of three creation methods.

### From Scratch

1. Click **Start From Scratch**.
2. Enter an **Intent Name** (required) and **Intent Description** (recommended). Click **+Add** for up to 5 secondary descriptions — each broadens semantic coverage and improves intent detection accuracy.

   <Note>Non-UTF-8 characters are not supported in the intent display name.</Note>

3. Set availability: **Customer Use Case**, **Agent AI Use Case**, or both.
   - **Agent AI** sub-settings: **During Chats**, **During Calls**, **During Emails**, **Launch through Library**.
4. Under **Intent Settings**, optionally configure:
   - **Sub-intent dialog** — task is invoked only as a subtask; not listed when users ask for help.
   - **Hide from Help** — task is not listed for help requests but remains directly invocable.
5. Under **Analytics - Containment Type**, select **Abandonment as Self-Service** or **Abandonment as Drop Off**.
6. Optionally configure **Conversation Context**, **Intent Preconditions**, or **Context Output**.
7. Click **Proceed**.

The Dialog Builder opens with a canvas containing the root intent.
...
2. Enter an **Intent Name** and a meaningful **Description**, then click **Generate**.
   ...
   <Note>A meaningful description is required. Without one, only an error prompt is generated.</Note>

3. Preview the generated dialog flow. Change the description and click **Regenerate** to iterate.
4. Click **Proceed**.

The platform auto-creates entities, prompts, error prompts, service tasks, and other parameters. Customize the flow as needed.

### From Marketplace Templates

<Note>Marketplace templates require the relevant integration to be installed in your AI Agent. Complete the installation journey to access integrations and templates.</Note>

1. Click **Marketplace**. Browse by **Categories** or **Integrations**. Installed integrations are labeled **Installed**.
2. Select an integration to see its available templates.
3. Choose an approach:

**Dialog Action Templates** — nodes preconfigured to make API calls for specific actions.

1. Click the integration → select the required **Action Template** → click **Install →**.
2. Open the created dialog in the builder to customize.

**Dialog Templates** — pre-built, end-to-end dialog flows for common scenarios.

<Note>Dialog templates are not available for every integration.</Note>

1. Click a **Dialog Template** tile to see its description, highlights, and flow preview.
2. Click **Install** → edit the name and description → click **Next**.
3. Complete the setup wizard (utterances, channel experience, intent-specific configuration).
4. Click **Finish**.

The dialog opens in the builder ready for customization.
