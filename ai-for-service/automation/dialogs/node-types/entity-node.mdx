...
2. Add the **Entity** node. See [Add a Node](../using-the-dialog-builder-tool.md).
...
4. Under **User Prompt**, enter the prompt shown to the user (for example, *Enter the Departure Date*).
   - Click **Manage** for channel-specific prompts. See [Prompt Editor](../prompt-editor.md).
...
5. Under **Error Prompts**, review and modify default error messages.
   - Click **Manage** to edit. See [User Prompts](../prompt-editor.md).
...
6. Toggle **Rephrase Responses** to rewrite agent replies using AI based on conversation context and user emotion. See [GenAI Features](../../../../generative-ai-tools/genai-features.md).
...
8. Enable **Sensitive Entity** to mask, redact, or replace sensitive input at the node level. Disabled by default. When enabled:
   ...
   See [Redacting PII Data](../../../../app-settings/advanced-settings/pii-data-masking.md).

9. Under **Variable Namespaces**, associate namespaces to scope this node's execution. Visible only when Variable Namespace is enabled for the agent. See [Managing Namespace](../../../../app-settings/managing-namespace.md).
...
| **Behavior on Exceeding Retries** | **End of Dialog** or **Transition to a Node**. Creates a "Behavior on Exceeding Retries" connection rule. Customize the message in [Standard Responses](../../../intelligence/conversation-management/standard-responses.md). |
...
| **Interruptions Behavior** | Use task-level setting \| Customize for this node. See [Interruption Handling](../../../intelligence/conversation-management/manage-interruptions.md). |
| **Analytics - Containment Type** | Use task-level default \| Customize (Self Service or Drop-off) |
| **Precedence** (all types except String/Description) | **Intent over Entity** or **Entity over Intent** when the user's input contains both. See [NLU Configurations](../../../natural-language/nlu-configurations/engine-tuning.md). |
| **Custom Tags** | Add tags for custom analytics profiles. See [Custom Meta Tags](../../../../analytics/automation/custom-dashboard/custom-meta-tags.md). |
...
1. Under **Suggested Synonyms**, enter synonyms for the entity and press **Enter** after each. See [Managing Synonyms](../../../natural-language/training/fundamental-meaning.md#manage-synonyms).
2. Under **Suggested Patterns**, click **+ Add Pattern** to add extraction patterns. See [Managing Patterns](../../../natural-language/training/fundamental-meaning.md#navigate-to-the-patterns-tab).
3. Under **Manage Context > Context Output**, define context tags set when this entity is populated.
4. Enable **Auto emits the entity values captured** to add entity values to the context object automatically. See [Context Management](../../../intelligence/context-management.md).

---

### IVR Properties

Configure input mode, grammar, prompts, and call behavior for IVR channels. See [Configure Dialog Node IVR Properties](../../../../channels/IVR-integration.md#configure-dialog-node-ivr-properties).
...
See [Adding IF-Else Conditions to Node Connections](../node-connections/nodes-conditions.md). To delete a condition, hover over it and click the bin icon. Deleted conditions cannot be restored.
