...
3. **Add the script node** in the designated place. For steps in adding nodes, [refer here](../using-the-dialog-builder-tool.md).
...
4. In the **Variable Namespaces** section, associate the variable namespaces to execute this node and its transitions. This option is visible only when the variable namespace is enabled for the AI Agent. You can go with the task level settings or customize it for this node. For more information, refer to [Managing Namespace](../../../../app-settings/managing-namespace.md).
...
2. Under the **Custom Tags** section, add tags to build custom profiles of your AI Agent's conversations. [Click here for more](../../../../analytics/automation/custom-dashboard/custom-meta-tags.md).
...
Using JavaScript, you can customize your dialog task by processing data before or after an API call — for example, directing the dialog task flow. You can use the `Context` object type-ahead feature to identify and select dynamic variables. For more information, refer to [Context Object](../../../intelligence/context-object.md).
...
For more information, refer to [Using Session and Context Variables in Tasks](../../using-session-and-context-variables.md).
...
* [koreUtil libraries](../../../../apis/automation/koreutil-libraries.md) — Pre-written JavaScript functions to make common or complex tasks easy to implement.
