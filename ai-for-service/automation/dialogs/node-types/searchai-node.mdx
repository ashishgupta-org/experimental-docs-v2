...
To set up node connection conditions, please follow the steps outlined in [Adding IF-Else Conditions to Node Connections](../node-connections/nodes-conditions.md). These settings apply only to this instance and will not affect other uses of the node. These settings are available only if this is the last node in a sequence.
