...
2. Click **GenAI** and then click **Search AI Node**. Alternatively, you can drag and drop the **Search AI Node** to the required location on the canvas. For more information on adding nodes, see [add a node](/ai-for-service/automation/agent-flows#adding-nodes).
...
To set up node connection conditions, please follow the steps outlined in [Adding IF-Else Conditions to Node Connections](/ai-for-service/automation/dialogs/node-types/node-connections#node-connections). These settings apply only to this instance and will not affect other uses of the node. These settings are available only if this is the last node in a sequence.
