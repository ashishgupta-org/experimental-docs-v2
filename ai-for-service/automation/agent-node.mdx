...
Controls how entity values flagged as PII appear in messages or responses. The global setting overrides the Agent Node configuration. Refer to [Redacting Personally Identifiable Information](/ai-for-service/app-settings/advanced-settings/pii-data-masking)
...
<Note>These options affect only the runtime display. PII data is always redacted in chat history and internal logs, regardless of this setting.</Note>
...
Define transition conditions from this node (instance-specific). See [Adding IF-Else Conditions to Node Connections](/ai-for-service/automatio/dialogs/node-types/node-connections#add-if--else-connection-conditions).
