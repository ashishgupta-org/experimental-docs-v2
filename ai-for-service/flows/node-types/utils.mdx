...
Use `agentUtils.setQueue()` to assign a queue at any point before agent transfer. Use it in a [Script Node](/ai-for-service/automation/dialogs/node-types/script-node#script-node) within a bot action or a [Script Task](/ai-for-service/automation/dialogs/node-types/script-task#script-task) in a Contact Center AI flow:
