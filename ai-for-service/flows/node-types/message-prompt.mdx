...
You can use a script to read data from an experience flow and pass it to a dialog in a child bot. The script node sets a variable from the current user session and stores it using `userSessionUtils`. [Learn more](../node-types/utils.md#pass-data-from-experience-flow-to-dialog-in-a-child-bot).
