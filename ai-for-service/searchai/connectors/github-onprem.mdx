...
For all content ingested from GitHub repositories, Search AI sets the `repository ID` as the `sys_racl` value. This value is stored as a permission entity. Use the [Permission Entity APIs](../../../apis/searchai/permission-entity-apis.mdx) to associate users with the permission entity corresponding to each repository ID.
