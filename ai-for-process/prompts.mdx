...
2. In the Prompt generator dialog, select a model and enter your instruction.
   ...
   ![Generate prompt](/images/generate_a_prompt.png)
...
2. In the Prompt library dialog, select a tab:
   ...
   ![Prompt library](/images/prompt_library_templates.png)

3. Click a template to preview it, then click **Use template**.

   ![Prompt library dialog](/images/prompt_template_dialog.png)

4. The template loads on the prompt canvas. Customize it as needed.

   ![Prompt library canvas](/images/prompt_canvas_library.png)
...
1. In the **System prompt** field, assign a role to the model. This field is optional—use the toggle to enable or disable it.
   ...
   ![System prompt](/images/promptcanvas-systemprompt.png)
...
2. In the **Prompt** field, enter your instructions. Click **Generate Prompt** to expand a short instruction into a more detailed prompt.
   ...
   ![Human prompt](/images/promptcanvas_human_prompt.png)
...
3. Optionally, define a **Response JSON schema** to structure model responses.
   ...
   ![JSON schema](/images/prompts_json.png)
...
2. In the **Variables** window, assign a value to each variable. Click **Add an empty row** to add multiple rows.
   ...
   ![Variables column](/images/promptcanvas_variable.png)

3. Verify that the variables are substituted correctly in the **Prompt** window.

   ![Prompt window with substituted variables](/images/prompt_window_example.png)
...
2. To add more models, select from the columns on the right.
   ...
   ![Select model](/images/prompt_studio_models.png)
...
![Output screen](/images/prompt_outputscreen_example.png)
...
![Model column options](/images/model_column_options.png)

| Icon                                                         | Feature               | Description                                                                                                      |
|--------------------------------------------------------------|-----------------------|------------------------------------------------------------------------------------------------------------------|
| ![Rearrange icon](/images/drag_drop.png)                     | Rearrange             | Drag and drop to reorder model columns on the canvas.                                                            |
| ![Remove model icon](/images/remove_model.png)               | Remove Model          | Remove a model from the comparison.                                                                              |
| ![Bookmark icon](/images/bookmark.png)                       | Bookmark a model      | Set a model as preferred. Required when committing a version if no model is bookmarked.                          |
| ![Model settings icon](/images/model_settings.png)           | Model Settings        | Adjust temperature, top k, top p, and max tokens. Defaults: top p: 1, top k: 5, temperature: 1, max tokens: 256. |
| ![Play icon](/images/play.png)                               | Play                  | Regenerate output for the entire column.                                                                         |
| ![Average response time icon](/images/avg_response_time.png) | Average Response Time | Average time the model takes to generate a response.                                                             |
| ![Average tokens icon](/images/avg_tokens.png)               | Average Tokens        | Mean number of input and output tokens used per interaction.                                                     |
| ![Copy icon](/images/copy.png)                               | Copy                  | Copy the generated output to clipboard.                                                                          |
| ![View in JSON icon](/images/view_in_json.png)               | View in JSON          | View the prompt and response in JSON format in a separate dialog.                                                |
| ![Regenerate icon](/images/regenerate.png)                   | Re-generate           | Regenerate output for a single cell.                                                                             |

### Top Toolbar Options

![Top toolbar options](/images/top_toolbar_options.png)

| Icon                                               | Feature                | Description                                                                    |
|----------------------------------------------------|------------------------|--------------------------------------------------------------------------------|
| ![Prompt library icon](/images/prompt_library.png) | Prompt library         | Browse and select from ~70 templates to use or customize.                      |
| ![Prompt API icon](/images/prompt_api.png)         | Prompt API             | Access version-specific API endpoints. Select cURL, Python, or Node.js format. |
| ![Three dots menu](/images/three_dots.png)         | Save to prompt library | Save the current prompt as a template in **My Templates**.                     |
| ![Three dots menu](/images/three_dots.png)         | Draft history          | Capture and restore the prompt canvas state at different points in time.       |
| ![Three dots menu](/images/three_dots.png)         | Export as CSV          | Export the canvas—inputs, outputs, and metadata—as a CSV file.                 |
| ![Three dots menu](/images/three_dots.png)         | Share                  | Share the prompt with other users for collaboration or reuse.                  |
| ![Versions icon](/images/versions.png)             | Versions               | View, compare, and restore prompt versions.                                    |
| ![Commit icon](/images/commit.png)                 | Commit                 | Save the current prompt as a new version (V1, V2, etc.).                       |
| ![Run icon](/images/run.png)                       | Run                    | Execute the prompt and generate output.                                        |
...
The system reads up to 10 rows from the CSV and maps column names to variables (case-sensitive). Ensure column names match variable names exactly—for example, `{{Name}}` requires a column named "Name." Data must start from the first row and column of the file.

<Note>You can import a maximum of 10 rows of data simultaneously.</Note>
...
Generate synthetic test datasets without manually creating data. The system analyzes your prompt and variables to produce contextually relevant values.

<Note>You can generate a maximum of 5 rows of data simultaneously.</Note>

<Note>Only OpenAI and Anthropic models are supported for test data generation.</Note>

1. In the prompt canvas, enter your prompt. For example: "Generate a job description with job title, experience, and location."
2. Add variables in double curly braces: `{{Job Title}}`, `{{Experience}}`, `{{Location}}`. The **Test data** option appears.
...
1. Click the **Bookmark model with its settings** icon on the model column.
   ...
   ![Bookmark model](/images/bookmark_model.png)
...
2. If you commit without bookmarking, a dialog prompts you to select a preferred model.
   ...
   ![Select preferred model](/images/prompt_bookmark_select_example.png)

3. After committing, click **Versions** to see the preferred model recorded in the version history.

   ![Versions dialog with preferred model](/images/versions_dialog_preferred_model.png)
...
![Draft history](/images/draft_history_new.png)
...
![Draft history dialog](/images/draft_history_dialog_new.png)
...
![Regenerate at cell level](/images/prompt_regenerate_cell_example.png)

**Column-level regeneration** — re-runs all rows for a model:

![Regenerate at model level](/images/prompt_regenerate_model_example.png)
...
1. Click **Commit** on the prompt canvas. The system saves the current prompt, system prompt, and preferred model as a new version.
   ...
   ![Committed version count](/images/prompt_no_of_versions_example.png)

2. Click **Versions** to view the version history.

   ![Prompt versions dialog](/images/prompt_versions_example.png)
...
1. Open the prompt and click the **three dots** icon > **Share**.
   ...
   ![Share option](/images/share_option.png)
...
![Prompt API keys](/images/prompt_api_keys.png)
