...
This API undeploys a model from the environment. The API response includes the model ID, model undeployment status, and the dockStatusId. After receiving the response, use the <code>dockStatusId</code> to call the [Get Dock Status API](/ai-for-process/apis/apis-list/get-dock-status) and verify successful model undeployment.
