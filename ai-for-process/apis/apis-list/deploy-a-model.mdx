...
The API response includes the **model ID** and the **model deployment status**. After receiving the response, use the <code>dockStatusId</code> to call the [Get Dock Status API](/ai-for-process/apis/apis-list/get-dock-status) and verify the successful deployment of the model.
