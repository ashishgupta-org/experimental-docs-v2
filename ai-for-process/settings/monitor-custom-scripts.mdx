...
For details on using the calendar widget, see [Time-based Audit Logs](/ai-for-process/settings/audit-logs#time-based-audit-logs).

### Column filtering

Apply column filters to narrow down runs or logs. This works similarly to the [Audit Logs filter](/ai-for-process/settings/audit-logs#filter-audit-logs), with an additional **contains** operator that matches results including a specific keyword or value.
...
Combine filters using **AND** or **OR** operators for multi-level filtering. AND and OR operators cannot be mixed in the same filter set. See [Add multiple filters](/ai-for-process/settings/audit-logs#add-multiple-filters) for details.
