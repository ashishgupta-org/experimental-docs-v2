...
| tags | Custom Tags related to user, session or message. Typically used in AI for Service app for analytics. Tag types: user, session, message. `{ "tags": { "message": { "Key": "Value" }, "user": { "Key": "Value" }, "session": { "Key": "Value" } } }` |
...
1. **Upload the file**. 
    ...
    Use the [File Upload API](/agent-platform/apis/agentic-apps/upload) to upload the desired files. Uploaded files remain available only for the duration of the session.
