...
When calling the Platform via the [Execute API](/agent-platform/apis/agentic-apps/execute), include a `metadata` field in each request to pass custom context. The Platform stores this in the `sessionMeta` memory store, making it accessible in agent prompts for personalized, context-aware responses.
...
[Learn more about the Execute API →](/agent-platform/apis/agentic-apps/execute)
...
[Learn more about the SessionMeta Memory Store →](/agent-platform/memory#session-meta-access)
