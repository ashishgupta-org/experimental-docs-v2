...
2. **The Platform processes uploaded files** in two ways:
    * **Content Extraction** — For [supported file types](../attachment-support.md#supported-file-formats-for-document-upload), the Platform extracts content and uses it as context.
    * **Metadata Storage** — For [all supported file types](../attachment-support.md#supported-file-formats-for-document-upload), the Platform captures and stores the following metadata in `sessionMeta.artifacts`:
        ...
        | `isActive` | Whether the document applies to the current conversation. [Learn more](../attachment-support.md#file-state-management). |
        | `downloadUrl` | Temporary direct access URL, **valid for 30 days** from date of upload |

3. **Metadata is accessible** in agent and supervisor prompts or code tools, and can be passed to third-party services for further processing. [Learn more](../attachment-support.md#file-metadata-extraction).
