...
**Import from Hugging Face**: Select your Hugging Face connection and enter the model name. See [Enable Hugging Face](/agent-platform/administration/integrations#hugging-face) for connection setup.
...
Connect your W&B account to monitor fine-tuning metrics in real-time. Select an existing connection or create a new one. See [Integrate with Weights & Biases](/agent-platform/administration/integrations#weights-&-biases-wandb).
