...
| **Response Display** | Shows the agent's response to each submitted query. If artifacts are enabled, the response includes artifacts after each message. They are displayed only when the `artifacts` array contains valid entries; empty arrays are ignored.  |
