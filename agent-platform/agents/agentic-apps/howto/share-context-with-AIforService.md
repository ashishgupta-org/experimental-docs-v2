...
2. Set Event Conditions: Configure the specific conditions that trigger the event. Use the **Description** field to specify the condition or logic that triggers the event (for example, fallback, confidence drop, intent match, explicit request). By default, this event is triggered whenever a conversation needs to be transferred to a human agent.
