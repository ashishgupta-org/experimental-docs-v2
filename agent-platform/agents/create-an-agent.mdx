...
- **Create an Agentic App.** Agents must be associated with an app and cannot exist independently. See [Create an Agentic App](./agentic-apps/create-an-app).
- **Configure an AI model** with tool-calling support. The platform supports OpenAI, Gemini, Anthropic, and Azure OpenAI. See [Add an External Model](./../models/external-models#add-a-model-via-easy-integration).
...
|--------|-------------|-------------------|
| [From Scratch](create-from-scratch) | You want full control over all settings | Profile, instructions, tools, and knowledge |
| [Connect External Agent](external-agents) | You have an agent hosted elsewhere | Connection details only |
| [From Marketplace](create-from-marketplace) | You want a preconfigured, ready-to-use agent | Minimal edits required |
