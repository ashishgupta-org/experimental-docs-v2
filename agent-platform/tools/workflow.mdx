...
- Once installed, the template appears in **My Tools**.
...
If a type mismatch occurs, the endpoint still succeeds but includes a warning in the response with the key name and nature of the mismatch. If one or more keys fail validation, the response includes warnings for failed keys and outputs for valid ones.
...
1. Click the **Run flow** icon at the top right corner of the canvas.
...
**Excluded:** API keys, sharing permissions, tool endpoint, audit logs.
...
<Note>The platform won't show the API key again after creation. Keep it secure and never expose it in client-side code or public repositories.</Note>
...
3. Click **Tool monitor** in the left navigation pane. The Tool monitor page is displayed.
...
Once associated, reference variables using:
