...
To avoid naming conflicts, imported tools are automatically prefixed with the MCP server name:

```
<MCP server name>__<tool name>
```

For example, a tool named `GMAIL_DELETE_DRAFT` imported from a server named `GoogleMCP` appears as `GoogleMCP__GMAIL_DELETE_DRAFT`.
